    ...
    >
      <div className="max-w-6xl mx-auto px-5 md:px-8 flex items-center justify-between h-[68px]">
        <Link href="/" className="text-2xl" style={{ fontFamily: "var(--font-script)" }}>
          <span className="text-cream">MVC</span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="text-sm font-medium text-cream/70 hover:text-gold transition-colors duration-200"
            >
              {link.label}
            </Link>
          ))}
            ...
            <nav className="flex flex-col px-6 py-6 gap-5">
              {navLinks.map((link) => (
                <Link
                  ...
                  className="text-lg font-medium text-cream/80 hover:text-gold transition-colors"
                >
                  {link.label}
                </Link>
              ))}
